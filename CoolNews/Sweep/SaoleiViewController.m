...
- (void)setupUI {
    ...
    [_left setFrame:CGRectMake(10, 10, 100, 30)];
    [_left setBackgroundColor:gMainColor];
...
- (void)winGame {
    ...
    if (array.count < 5 || (array.count >= 5 && ((SweepUserModel *)array.lastObject).time > self.timeInterval)) {
        message = [NSString stringWithFormat:@"你赢得了%@模式,完成比赛所用时间为%ld秒,进入了记录榜哦！！！",difficulty,self.timeInterval];
        
        SweepUserModel *model = array.lastObject;
